...
export class WorkspacesService {
  ...
  public static async downloadAll(sessionId: string): Promise<{ blob: Blob; filename: string }> {
    const token =
      typeof OpenAPI.TOKEN === 'function'
        ? await OpenAPI.TOKEN({ method: 'GET', url: '' } as any)
        : (OpenAPI.TOKEN ?? '')
    const resp = await fetch(
      `${OpenAPI.BASE}/api/v1/workspace/${encodeURIComponent(sessionId)}/download-all`,
      {
        method: 'GET',
        headers: token ? { Authorization: `Bearer ${token}` } : {},
        credentials: OpenAPI.WITH_CREDENTIALS ? 'include' : 'same-origin',
