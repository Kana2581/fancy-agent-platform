...
export class KnowledgeGraphService {
    ...
    // ---- Nodes ----
    public static listNodes(graphId: number, search?: string, type?: string): CancelablePromise<KGNodeOut[]> {
        return __request(OpenAPI, {
            method: 'GET',
    ...
    public static async exportCypher(graphId: number): Promise<string> {
        ...
        const resp = await fetch(`${OpenAPI.BASE}/api/v1/knowledge-graph/graphs/${graphId}/export/cypher`, {
            method: 'GET',
    ...
    // ---- Extract ----
    public static extractFromText(graphId: number, text: string, agentId: number): CancelablePromise<KGExtractPreview> {
        return __request(OpenAPI, {
            method: 'POST',
