...
function toolToForm(t: ApiToolOut): WizardForm {
  const headerPairs: HeaderPair[] = Object.entries(t.headers || {}).map(([key, value]) => ({
    key,
    value,
  }))
  return {
...
function formToCreate(f: WizardForm): ApiToolCreate {
  ...
  try {
    fixed_params = JSON.parse(f.fixed_params_json)
  } catch {
    /* ignore */
  }
...
const inputCls =
  'w-full px-4 py-3 bg-white dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-xl focus:ring-1 focus:ring-gray-300 dark:focus:ring-zinc-600 focus:border-gray-500 dark:focus:border-zinc-400 outline-none transition-all text-gray-800 placeholder-gray-500 text-sm'
const selectCls =
  'w-full px-4 py-3 bg-white dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-2xl focus:ring-2 focus:ring-gray-400 dark:focus:ring-zinc-500/50 outline-none transition-all text-gray-800 dark:text-white text-sm'
      ...
      {steps.map((label, i) => {
        ...
        return (
          <React.Fragment key={num}>
            ...
            >
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${onJump ? 'group-hover:ring-2 group-hover:ring-gray-300 dark:group-hover:ring-zinc-600 cursor-pointer' : ''} ${
                  active
            ...
            {i < steps.length - 1 && (
              <div
                className={`flex-1 h-0.5 mx-2 rounded transition-all ${done ? 'bg-green-400/60' : 'bg-gray-100 dark:bg-zinc-800'}`}
              />
            )}
          </React.Fragment>
        )
...
function Step2({ form, onChange }: { form: WizardForm; onChange: (f: WizardForm) => void }) {
  ...
  const removeHeader = (i: number) =>
    onChange({ ...form, headers: form.headers.filter((_, j) => j !== i) })

  const applyBearer = () =>
    onChange({
      ...form,
      headers: [
        ...form.headers.filter((h) => h.key !== 'Authorization'),
  ...
  const applyApiKey = () =>
    onChange({
      ...form,
      headers: [
        ...form.headers.filter((h) => h.key !== 'X-API-Key'),
        { key: 'X-API-Key', value: 'YOUR_KEY' },
      ],
    })
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex gap-2">
            <button
              onClick={applyBearer}
              className="px-2.5 py-1 text-xs bg-purple-400/20 hover:bg-purple-400/30 text-gray-700 dark:text-zinc-300 rounded-lg transition-all border border-purple-300/30"
            >
              Bearer Token
            </button>
            <button
              onClick={applyApiKey}
              className="px-2.5 py-1 text-xs bg-orange-400/20 hover:bg-orange-400/30 text-orange-800 rounded-lg transition-all border border-orange-300/30"
            >
              API Key
            </button>
            <button
              onClick={addHeader}
              className="px-2.5 py-1 text-xs bg-gray-100 dark:bg-zinc-800 hover:bg-gray-200 dark:hover:bg-zinc-600 text-gray-700 rounded-lg transition-all border border-gray-200 dark:border-zinc-700 flex items-center gap-1"
            >
              <Plus size={11} />
              添加
            </button>
...
function Step3({ form, onChange }: { form: WizardForm; onChange: (f: WizardForm) => void }) {
  ...
  const removeParam = (i: number) =>
    onChange({ ...form, tool_params: form.tool_params.filter((_, j) => j !== i) })
  ...
  return (
    <div className="space-y-4">
      ...
      {form.tool_params.map((p, i) => (
        <div
          key={i}
          className="p-4 bg-white dark:bg-zinc-900 rounded-2xl border border-gray-200 dark:border-zinc-700 space-y-3"
        >
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-gray-700">参数 #{i + 1}</span>
            <button
              onClick={() => removeParam(i)}
              className="p-1.5 hover:bg-red-100/50 text-red-500 rounded-lg transition-all"
            >
              <Trash2 size={14} />
            </button>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                className={inputCls}
                placeholder="模型使用的名称（如 city）"
                value={p.name}
                onChange={(e) =>
                  updateParam(i, { name: e.target.value, path: p.path || e.target.value })
                }
              />
            </div>
            <div>
              ...
              <input
                className={inputCls}
                placeholder="dot-path（如 filter.city）"
                value={p.path}
            ...
            <div className="flex items-end pb-1">
              <label className="flex items-center gap-2 cursor-pointer select-none">
                <input
                  ...
                  className="w-4 h-4 rounded text-gray-600 dark:text-zinc-300 focus:ring-gray-400 dark:focus:ring-zinc-500"
                />
...
function Step4({
  ...
  const addExtract = () =>
    onChange({ ...form, response_extract: [...form.response_extract, { path: '', alias: '' }] })
  ...
  const removeExtract = (i: number) =>
    onChange({ ...form, response_extract: form.response_extract.filter((_, j) => j !== i) })
    ...
    <div className="space-y-5">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            onClick={addExtract}
            className="px-2.5 py-1 text-xs bg-gray-100 dark:bg-zinc-800 hover:bg-gray-200 dark:hover:bg-zinc-600 text-gray-700 rounded-lg border border-gray-200 dark:border-zinc-700 flex items-center gap-1"
          >
            <Plus size={11} />
            添加
          </button>
        ...
        <div className="space-y-2">
          {form.response_extract.map((ex, i) => (
            <div key={i} className="flex gap-2 items-center">
              <input
                className={inputCls + ' flex-1'}
                placeholder="提取路径（如 data.items[*]）"
                value={ex.path}
                onChange={(e) => updateExtract(i, { path: e.target.value })}
              />
              <input
                className={inputCls + ' flex-1'}
                placeholder="别名（如 items）"
                value={ex.alias}
                onChange={(e) => updateExtract(i, { alias: e.target.value })}
              />
              <button
                onClick={() => removeExtract(i)}
                className="p-2 hover:bg-red-100/50 text-red-500 rounded-xl"
              >
                <Trash2 size={15} />
              </button>
            </div>
          ))}
          {form.response_extract.length === 0 && (
            <p className="text-xs text-gray-500 italic">不配置则返回完整响应</p>
          )}
        </div>
      </div>
      ...
      <div>
        ...
        <input
          ...
          max={50000}
          onChange={(e) =>
            onChange({ ...form, response_max_chars: parseInt(e.target.value) || 2000 })
          }
        />
      </div>
      ...
      <div className="border-t border-gray-200 dark:border-zinc-800 pt-4">
        ...
        {toolId ? (
          <div className="space-y-3">
            ...
            {testResult && (
              <div
                className={`rounded-xl border overflow-hidden ${testResult.success ? 'border-emerald-500/40' : 'border-red-500/40'}`}
              >
                <div
                  className={`px-3 py-1.5 text-xs font-medium ${testResult.success ? 'bg-green-100 dark:bg-green-900/20 text-green-800 dark:text-green-300' : 'bg-red-500/30 text-red-900'}`}
                >
                  {testResult.success ? '✓ 请求成功' : '✗ 请求失败'}
                </div>
                <pre
                  className={`p-3 text-xs font-mono overflow-auto max-h-40 text-gray-800 ${testResult.success ? 'bg-emerald-400/10' : 'bg-red-400/10'}`}
                >
          ...
          <p className="text-xs text-gray-500 bg-gray-50 dark:bg-zinc-900 rounded-xl p-3 border border-gray-200 dark:border-zinc-800">
            保存工具后，在编辑界面即可使用测试功能
          </p>
        )}
      </div>
...
const ApiToolWizard: React.FC<ApiToolWizardProps> = ({ initialTool, onSave, onCancel }) => {
  ...
  const validate = (s: number): string[] => {
    ...
    if (s === 2) {
      ...
      try {
        JSON.parse(form.fixed_params_json)
      } catch {
        errs.push('固定参数 JSON 格式错误')
      }
    }
    if (s === 3) {
