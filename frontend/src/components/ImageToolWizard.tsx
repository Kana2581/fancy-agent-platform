...
const ImageToolWizard: React.FC<Props> = ({ initialTool, onSave, onCancel }) => {
  ...
  const inputClass =
    'w-full px-4 py-2.5 bg-gray-100 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-xl text-gray-800 placeholder-gray-500 outline-none focus:ring-2 focus:ring-gray-400 dark:focus:ring-zinc-500/50'
  const selectClass =
    'w-full px-4 py-2.5 bg-gray-100 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-xl text-gray-800 outline-none focus:ring-2 focus:ring-gray-400 dark:focus:ring-zinc-500/50'
            ...
            <>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <ThemedSelect
                    value={form.default_size ?? '1024x1024'}
                ...
                <div>
                  ...
                  <ThemedSelect
                    value={form.default_quality ?? 'standard'}
                    onChange={(v) => set('default_quality', v)}
                    options={OPENAI_QUALITIES.map((q) => ({ value: q, label: q }))}
                    className={selectClass}
                ...
                <div>
                  ...
                  <ThemedSelect
                    value={form.default_style ?? 'vivid'}
                    onChange={(v) => set('default_style', v)}
                    options={OPENAI_STYLES.map((s) => ({ value: s, label: s }))}
                    className={selectClass}
                    disabled={form.model === 'dall-e-2'}
                  />
                </div>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    type="number"
                    value={(form.extra_params as Record<string, number>)?.steps ?? 30}
                    onChange={(e) => setExtra('steps', Number(e.target.value))}
                    min={10}
                    max={150}
                    className={inputClass}
                  />
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={(form.extra_params as Record<string, number>)?.cfg_scale ?? 7}
                    onChange={(e) => setExtra('cfg_scale', Number(e.target.value))}
                    min={1}
                    max={35}
                    step={0.5}
                    className={inputClass}
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={(form.extra_params as Record<string, number>)?.num_inference_steps ?? 20}
                    onChange={(e) => setExtra('num_inference_steps', Number(e.target.value))}
                    min={1}
                    max={100}
                    className={inputClass}
                  />
              ...
              <div>
                ...
                <input
                  type="number"
                  value={(form.extra_params as Record<string, number>)?.guidance_scale ?? 7.5}
                  onChange={(e) => setExtra('guidance_scale', Number(e.target.value))}
                  min={1}
                  max={20}
                  step={0.5}
                  className={inputClass}
            ...
            <>
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  推荐：qwen-image-2.0-pro / qwen-image-2.0
                </p>
              </div>
              <div>
          ...
          <div className="mt-4 p-4 bg-gray-50 dark:bg-zinc-900 rounded-2xl border border-gray-200 dark:border-zinc-800">
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="输入测试提示词..."
                className="flex-1 px-4 py-2 bg-gray-100 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-xl text-gray-800 placeholder-gray-500 outline-none focus:ring-2 focus:ring-gray-400 dark:focus:ring-zinc-500/50 text-sm"
              />
              <button
                onClick={handleTest}
                disabled={testLoading || !testPrompt.trim() || !savedToolId || !form.model}
            ...
            {testImage && (
              <div className="mt-3">
                <img
                  src={testImage}
                  alt="生成结果"
                  className="max-w-full rounded-xl border border-gray-200 dark:border-zinc-800"
                />
              </div>
            )}
          </div>
