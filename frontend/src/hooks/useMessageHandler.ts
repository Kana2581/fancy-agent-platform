...
interface UseMessageHandlerReturn {
  ...
  handleSendMessage: (message: string, fileIds?: number[], files?: ChatResponse['files']) => Promise<void>;
...
export const useMessageHandler = ({ sessionId }: UseMessageHandlerProps): UseMessageHandlerReturn => {
    ...
    setAllMessages((prev) => {
      ...
      if (type === 'AIMessageChunk') {
        ...
        if (index !== -1) {
          ...
          updated[index] = {
            ...updated[index],
            ...(content && { content: updated[index].content + content }),
  ...
  function handleStreamMessage(raw: string) {
    ...
    handleStreamChunk(chunk.id, parsed.type, chunk.content, chunk.tool_calls, chunk.parent_id, chunk.usage_metadata);
  }
