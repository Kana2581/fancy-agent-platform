...
const SettingsPage: React.FC = () => {
  ...
  const { llmModels, setLLMModels, mcpTools, setMCPTools, setSelectedSession, refreshLLMs, refreshMCPs } = useAppContext();
  ...
  const saveLLM = async () => {
    if (editingLLMItem) {
      const updated = await LlmModelsService.updateLlmApiV1LlmLlmIdPut(
        editingLLMItem.id,
        llmForm
      );
  ...
  const saveMCP = async () => {
    if (editingMCPItem) {
      const updated = await McpService.updateMcpApiV1McpsMcpIdPut(
        editingMCPItem.id,
        mcpForm
      );
