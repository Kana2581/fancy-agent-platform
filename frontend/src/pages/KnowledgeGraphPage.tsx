...
const TYPE_COLORS: Record<string, string> = {
  ...
  product:
    'bg-gray-200 dark:bg-zinc-700 text-gray-600 dark:text-zinc-400 border-gray-300 dark:border-zinc-600',
}
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ color: dimmed ? `${color}50` : color }}
      >
        {type}
      </div>
      <div
        className="text-base font-bold leading-tight"
        style={{ color: dimmed ? 'rgba(248,250,252,0.35)' : '#f8fafc' }}
      >
        {label}
      </div>
...
function toFlowData(
  ...
  const visibleIds = new Set(
    kgNodes.filter((n) => !hiddenTypes.has(n.type)).map((n) => String(n.id))
  )
...
function JumpToNode({ nodeId, rfNodes }: { nodeId: string | null; rfNodes: Node[] }) {
  ...
  useEffect(() => {
    ...
    if (n)
      void setCenter(n.position.x + NODE_WIDTH / 2, n.position.y + NODE_HEIGHT / 2, {
        zoom: 1.5,
        duration: 600,
      })
...
const KGGraphView: React.FC<KGGraphViewProps> = ({ kgNodes, kgEdges }) => {
  ...
  return (
    <div className="flex-1 p-4 overflow-hidden">
      <div
        className="w-full h-full rounded-xl overflow-hidden border border-gray-200 dark:border-zinc-800"
      ...
      >
        ...
        >
          ...
          <MiniMap
            nodeColor={(n) => dotColor(n.data.type as string)}
          ...
          <Panel position="top-left" style={{ margin: 12 }}>
            <div
              className="flex items-center gap-2 px-3 py-2 rounded-2xl border border-gray-200 dark:border-zinc-800"
          ...
          >
            ...
            <div
              className="flex gap-1 p-1 rounded-2xl border border-gray-200 dark:border-zinc-800"
              style={{ background: 'rgba(10,15,30,0.85)', backdropFilter: 'blur(20px)' }}
            >
              <button
            ...
            {allTypes.length > 0 && (
              <div
                className="flex flex-col gap-0.5 p-2 rounded-2xl border border-gray-200 dark:border-zinc-800"
                style={{ background: 'rgba(10,15,30,0.85)', backdropFilter: 'blur(20px)' }}
              >
                {allTypes.map((type) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        ...
                        style={{ background: dotColor(type) }}
                      />
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-50 dark:bg-zinc-900  rounded-xl shadow-sm border border-gray-200 dark:border-zinc-700 w-full max-w-md">
        <div className="flex items-center justify-between p-6 pb-4 border-b border-gray-200 dark:border-zinc-800">
          ...
          <button onClick={onClose} className="p-2 hover:bg-black/10 rounded-xl transition">
            <X size={20} />
          </button>
        </div>
        <div className="px-6 pb-6 pt-4 space-y-4">
          ...
          <div className="flex gap-3 pt-1">
            <button
              onClick={onClose}
              className="flex-1 py-2.5 bg-gray-200 dark:bg-zinc-700 hover:bg-white/40 text-gray-700 rounded-2xl font-medium transition"
            >
              取消
            </button>
            <button
...
const EdgeModal: React.FC<EdgeModalProps> = ({ nodes, onClose, onSave }) => {
  ...
  const selectClass =
    'w-full px-4 py-2.5 bg-gray-100 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-2xl focus:outline-none focus:ring-2 focus:ring-gray-400 dark:focus:ring-zinc-500/50 text-gray-800'
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-50 dark:bg-zinc-900  rounded-xl shadow-sm border border-gray-200 dark:border-zinc-700 w-full max-w-md">
        ...
        <div className="px-6 pb-6 pt-4 space-y-4">
          <div>
            <label className="text-base font-medium text-gray-700 mb-1 block">
              起点实体 <span className="text-red-500">*</span>
            </label>
            <select
              value={sourceId}
          ...
          <div>
            <label className="text-base font-medium text-gray-700 mb-1 block">
              关系标签 <span className="text-red-500">*</span>
            </label>
            <input
              value={relation}
          ...
          <div>
            <label className="text-base font-medium text-gray-700 mb-1 block">
              终点实体 <span className="text-red-500">*</span>
            </label>
            <select
              value={targetId}
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button
              onClick={handleSave}
              disabled={saving || !sourceId || !targetId || !relation.trim()}
              className="flex-1 py-2.5 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-2xl font-medium disabled:opacity-50 transition"
            >
              {saving ? '保存中...' : '添加'}
            </button>
...
const ExtractModal: React.FC<ExtractModalProps> = ({ graphId, agents, onClose, onConfirm }) => {
  ...
  const handleExtract = async () => {
    ...
    setPreview(null)
    try {
      const result = await KnowledgeGraphService.extractFromText(
        graphId,
        text.trim(),
        Number(agentId)
      )
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-50 dark:bg-zinc-900  rounded-xl shadow-sm border border-gray-200 dark:border-zinc-700 w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          <div>
            <label className="text-base font-medium text-gray-700 mb-1 block">
              使用 Agent 的 LLM <span className="text-red-500">*</span>
            </label>
            <select
              value={agentId}
          ...
          <button
            ...
            disabled={extracting || !text.trim() || !agentId}
          ...
          {error && (
            <p className="text-sm text-red-500 bg-red-50/50 rounded-xl px-4 py-2">{error}</p>
          )}
                ...
                {preview.nodes.length === 0 ? (
                  ...
                  preview.nodes.map((n, i) => (
                    <div key={i} className="flex items-center gap-2 text-sm">
                      <span
                        className={`px-2.5 py-0.5 rounded-full border text-xs font-semibold ${typeColor(n.type)}`}
                      >
                        {n.type}
                      </span>
                      <span className="text-gray-800 font-semibold">{n.name}</span>
                      {n.description && (
                        <span className="text-gray-500 text-sm truncate">— {n.description}</span>
                      )}
                    </div>
                  ))
                )}
                ...
                {preview.edges.length === 0 ? (
                  ...
                  preview.edges.map((e, i) => (
                    <div key={i} className="flex items-center gap-1.5 text-sm text-gray-700">
                      ...
                      <ChevronRight
                        size={13}
                        className="text-gray-600 dark:text-zinc-300 shrink-0"
                      />
                      <span className="px-2.5 py-0.5 rounded-full bg-gray-100 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 text-gray-600 dark:text-zinc-300 text-xs font-semibold">
                        {e.relation}
                      </span>
                      <ChevronRight
                        size={13}
...
const KnowledgeGraphPage: React.FC = () => {
  ...
  // Modals
  const [nodeModal, setNodeModal] = useState<{ open: boolean; editing: KGNodeOut | null }>({
    open: false,
    editing: null,
  })
  ...
  useEffect(() => {
    Promise.all([KnowledgeGraphService.listGraphs(), AgentsService.listAgentsApiV1AgentsGet()])
      .then(([gs, as]) => {
        setGraphs(gs)
        setAgents(as)
  ...
  useEffect(() => {
    if (selectedGraph) {
      ...
      setTypeFilter('')
      setTab('nodes')
      void loadContent(selectedGraph)
    }
  ...
  const handleNodeSave = async (data: KGNodeCreate | KGNodeUpdate) => {
    ...
    if (nodeModal.editing) {
      const updated = await KnowledgeGraphService.updateNode(
        nodeModal.editing.id,
        data as KGNodeUpdate
      )
  ...
  const handleExtractConfirm = async (preview: KGExtractPreview) => {
    ...
    for (const n of preview.nodes) {
      try {
        ...
        const created = await KnowledgeGraphService.createNode(selectedGraph.id, {
          name: n.name,
          type: n.type,
    ...
    for (const e of preview.edges) {
      ...
      try {
        const created = await KnowledgeGraphService.createEdge(selectedGraph.id, {
  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      <div
        className={`shrink-0 flex flex-col border-r border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-800/30 overflow-hidden transition-all duration-300 ${sidebarCollapsed ? 'w-12' : 'w-60'}`}
      >
        <div className="p-3 border-b border-gray-200 dark:border-zinc-800 flex items-center justify-between shrink-0">
          ...
          <div
            className={`flex items-center gap-1 ${sidebarCollapsed ? 'w-full justify-center' : ''}`}
          >
            {!sidebarCollapsed && (
              <button
                onClick={() => {
            ...
            {creating && (
              <div className="flex items-center gap-1 p-2 bg-gray-100 dark:bg-zinc-800 rounded-xl border border-gray-200 dark:border-zinc-700">
                ...
                <button
                  ...
                  className="p-1 hover:bg-gray-200 dark:hover:bg-zinc-600 rounded-lg transition disabled:opacity-40"
                >
                ...
                >
                  {editingGraph?.id === g.id ? (
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') void handleUpdateGraph(g)
                        if (e.key === 'Escape') setEditingGraph(null)
                      }}
                    ...
                    <span className="flex-1 text-base text-gray-800 truncate font-medium">
                      {g.name}
                    </span>
                  )}
                  <div
            ...
            <div className="px-6 pt-5 pb-4 border-b border-gray-200 dark:border-zinc-800 flex items-center justify-between shrink-0">
              <div>
                ...
                <div className="flex items-center gap-4 mt-1">
                  ...
                  {allTypes.length > 0 && (
                    <span className="text-sm text-gray-500">{allTypes.length} 种类型</span>
                  )}
                </div>
              </div>
              <div className="flex gap-2">
                ...
                <button
                  onClick={() =>
                    tab === 'edges'
                      ? setEdgeModal(true)
                      : setNodeModal({ open: true, editing: null })
                  }
                  className="px-4 py-2 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-2xl hover:shadow-lg transition flex items-center gap-2 text-sm font-medium"
                >
                  <Plus size={15} />
                  ...
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${
                      tab === t
                        ...
                        : 'text-gray-600 hover:bg-gray-100 dark:hover:bg-zinc-700'
                    }`}
                  >
                      ...
                      {filteredNodes.map((node) => (
                        ...
                        >
                          <div className="flex items-start justify-between">
                            ...
                            <span
                              className={`ml-2 shrink-0 text-xs px-2.5 py-1 rounded-full border font-semibold ${typeColor(node.type)}`}
                            >
                              {node.type}
                            </span>
                          </div>
