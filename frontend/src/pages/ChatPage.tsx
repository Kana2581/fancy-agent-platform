...
import { Send, Bot, Loader2, Paperclip, X, MessageCirclePlus, Archive, Download, AlertCircle, Wrench, Share2, Copy, Check, Eye, EyeOff } from 'lucide-react';
import { useNavigate, useParams } from 'react-router-dom';
import type { AgentFullOut, SimpleFile, MCPOut, ApiToolOut, ImageToolOut, BuiltinToolInfo, ChatResponse } from '../api';
import { AgentApiToolsService, AgentBuiltinToolsService, AgentImageToolsService, AgentMcpsService, AgentsService, ApiToolsService, BuiltinToolsService, ChatService, FilesService, ImageToolsService, McpService, SessionsService, SessionSharesService } from '../api';
...
const ChatPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      AgentBuiltinToolsService.listAgentBuiltinToolsApiV1AgentsAgentIdBuiltinToolsGet(selectedAgent.id),
    ]).then(([allMcps, allApiTools, allImageTools, allBuiltins, mcpIds, apiToolIds, imageToolIds, builtinTypes]) => {
      ...
      setEnabledMCPCount(mcpIds.length + apiToolIds.length + imageToolIds.length + builtinTypes.length);
    });
  }, [selectedAgent]);
  ...
  const updateEnabledCount = (mcpIds: number[], apiIds: number[], imageIds: number[], builtinTypes: string[]) => {
  ...
  const toggleApiTool = async (toolId: number) => {
    ...
    await AgentApiToolsService.syncAgentToolsApiV1AgentsAgentIdApiToolsPost(selectedAgent.id, { tool_ids: newIds });
  };
  ...
  const toggleImageTool = async (toolId: number) => {
    ...
    await AgentImageToolsService.syncAgentImageToolsApiV1AgentsAgentIdImageToolsPost(selectedAgent.id, { tool_ids: newIds });
  };
  ...
  const toggleBuiltinTool = async (toolType: string) => {
    ...
    await AgentBuiltinToolsService.syncAgentBuiltinToolsApiV1AgentsAgentIdBuiltinToolsPost(selectedAgent.id, { tool_types: newTypes });
  };
  ...
  const uploadSingleFile = (file: File) => {
    ...
    setSelectedFiles((prev) => [...prev, { localId, file, status: 'uploading', previewUrl, request }]);
    ...
    <>
      ...
      <div className="min-h-20 bg-gray-50 dark:bg-zinc-900 border-b border-gray-200 dark:border-zinc-800 flex items-center justify-between gap-4 px-6 py-4 shadow-sm">
        <div className="flex min-w-0 items-center gap-4">
          <div className="shrink-0 text-4xl">{hasSession ? selectedAgent?.avatar || '🤖' : '💬'}</div>
          ...
          {displayMessages.length > 0 && (
            <>
              ...
              <button
                ...
                aria-label="压缩对话"
              >
                {isCompressing ? <Loader2 size={16} className="animate-spin" /> : <Archive size={16} />}
              </button>
            </>
          )}
          <button
        ...
        {hasSession ? (
          <div className="max-w-3xl mx-auto space-y-6">
            {(() => {
              ...
              const isIntermediate = (m: ChatResponse) =>
                m.type === 'tool' ||
                (m.type === 'ai' && m.name !== '__compressed__' && Array.isArray(m.tool_calls) && m.tool_calls.length > 0);
          ...
          <div className="max-w-2xl mx-auto h-full flex items-center">
            <div className="w-full bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl p-10 text-center shadow-lg">
              ...
              <h3 className="text-2xl font-semibold text-gray-800 mb-3">先创建一个 Agent 再开始聊天</h3>
              <p className="text-gray-600 mb-6">你现在在纯聊天入口（/chat），还没有具体会话。请先去 Agents 页面创建或选择 Agent，然后再点击“开始对话”。</p>
              <button
                onClick={() => navigate('/agents')}
      ...
      {/* 输入区域 */}
      {hasSession && <div className="bg-gray-50 dark:bg-zinc-900 border-t border-gray-200 dark:border-zinc-800 px-6 pt-4 pb-3">
        <div
        ...
        >
          ...
          {streamError && (
            <div className="mb-4 flex items-center gap-3 px-4 py-3 bg-red-500/15 rounded-2xl border border-red-400/30 text-red-400">
              ...
              <button onClick={clearStreamError} className="p-1 hover:bg-red-400/20 rounded-lg transition-colors">
                <X size={14} />
              </button>
            </div>
          )}
            ...
            {showToolPanel && (
              <div className="absolute bottom-full mb-3 left-0 w-80 max-h-[60vh] overflow-y-auto bg-white dark:bg-slate-800/60 rounded-xl border border-gray-200 dark:border-white/15 shadow-lg z-50 p-4 space-y-4">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm font-semibold text-gray-800 dark:text-white">工具选择</span>
                  <span className="text-xs text-gray-500 dark:text-white/50">{enabledMCPCount} 个已启用</span>
                </div>

                {/* MCP 工具 */}
                {availableMcps.length > 0 && (
                  <div>
                    <p className="text-xs font-medium text-gray-500 dark:text-white/50 mb-2 px-1">MCP 工具</p>
                    <div className="space-y-1">
                      {availableMcps.map(mcp => (
                        ...
                        >
                          ...
                          />
                          <span className="text-sm text-gray-800 dark:text-white truncate">{mcp.mcp_name}</span>
                        </label>
                      ))}
                    </div>
                  </div>
                ...
                {availableApiTools.length > 0 && (
                  <div>
                    <p className="text-xs font-medium text-gray-500 dark:text-white/50 mb-2 px-1">HTTP API 工具</p>
                    <div className="space-y-1">
                      {availableApiTools.map(tool => (
                        ...
                        >
                          <input
                            ...
                            className="w-4 h-4 rounded text-gray-600 dark:text-zinc-300 focus:ring-gray-400 dark:focus:ring-zinc-500 focus:ring-2"
                          />
                          <span className="text-sm text-gray-800 dark:text-white flex-1 truncate">{tool.name}</span>
                          {tool.method && (
                            <span className="text-xs text-gray-500 dark:text-white/60 bg-gray-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded-full shrink-0">{tool.method}</span>
                          )}
                        </label>
                      ))}
                    </div>
                ...
                {availableImageTools.length > 0 && (
                  <div>
                    <p className="text-xs font-medium text-gray-500 dark:text-white/50 mb-2 px-1">图像生成工具</p>
                    <div className="space-y-1">
                      {availableImageTools.map(tool => (
                        ...
                        >
                          ...
                          />
                          <span className="text-sm text-gray-800 dark:text-white flex-1 truncate">{tool.name}</span>
                          <span className="text-xs text-gray-500 dark:text-white/60 bg-gray-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded-full shrink-0">{tool.provider}</span>
                        </label>
                      ))}
                    </div>
                  </div>
                ...
                {availableBuiltinTools.length > 0 && (
                  <div>
                    <p className="text-xs font-medium text-gray-500 dark:text-white/50 mb-2 px-1">内置工具</p>
                    <div className="space-y-1">
                      {availableBuiltinTools.map(tool => (
                        ...
                        >
                          ...
                          />
                          <div className="flex-1 min-w-0">
                            <span className="text-sm text-gray-800 dark:text-white block truncate">{tool.name}</span>
                            {tool.description && (
                              <span className="text-xs text-gray-500 dark:text-white/60 block truncate">{tool.description}</span>
                            )}
                          </div>
                        </label>
                      ))}
                ...
                {availableMcps.length === 0 && availableApiTools.length === 0 && availableImageTools.length === 0 && availableBuiltinTools.length === 0 && (
                  <p className="text-sm text-gray-500 dark:text-white/50 text-center py-2">暂无可用工具</p>
                )}
              </div>
            )}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-50 dark:bg-zinc-900 rounded-xl shadow-sm border border-gray-200 dark:border-zinc-700 w-full max-w-md">
            <div className="flex items-center justify-between p-6 pb-4 border-b border-gray-200 dark:border-zinc-800">
              ...
              <button onClick={() => setShareModalOpen(false)} className="p-2 hover:bg-black/10 rounded-xl transition">
                <X size={20} />
              </button>
            </div>
              ...
              {!shareUrl ? (
                ...
                <>
                  ...
                  <div className="flex items-center gap-2 bg-white dark:bg-zinc-900 rounded-2xl border border-gray-200 dark:border-zinc-800 px-3 py-2">
                    ...
                    <button onClick={handleCopyShareUrl} className="p-1.5 hover:bg-gray-200 dark:hover:bg-zinc-600 rounded-lg transition">
                      {shareCopied ? (
