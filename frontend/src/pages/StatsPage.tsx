      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            {[
              { label: '总输出 Tokens', value: formatNum(summary?.total_output_tokens ?? 0), color: '' },
              { label: '总输入 Tokens', value: formatNum(summary?.total_input_tokens ?? 0), color: 'from-gray-600 to-gray-900' },
              { label: 'AI 消息数', value: String(summary?.total_messages ?? 0), color: 'from-gray-500 to-gray-800' },
              { label: '会话数', value: String(summary?.total_sessions ?? 0), color: 'from-indigo-400 to-purple-500' },
            ].map((card) => (
              <div
                key={card.label}
              ...
              >
                ...
                <div className={`text-3xl font-bold bg-${card.color} bg-clip-text text-transparent`}>
                  {card.value}
                </div>
              </div>
            ))}
            ...
            {daily.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={240}>
                <LineChart data={daily} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip
                    contentStyle={{ background: 'rgba(255,255,255,0.85)', borderRadius: 12, border: 'none', fontSize: 12 }}
                    formatter={(value: unknown) => [formatNum(Number(value ?? 0)), '']}
                  />
                  <Legend wrapperStyle={{ fontSize: 12, color: tickColor }} />
                  <Line type="monotone" dataKey="output_tokens" name="输出 Tokens" stroke="#06b6d4" strokeWidth={2} dot={false} />
                  <Line type="monotone" dataKey="input_tokens" name="输入 Tokens" stroke="#6366f1" strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            )}
          </div>
            ...
            {byAgent.length === 0 ? (
              ...
              <>
                <ResponsiveContainer width="100%" height={Math.max(200, byAgent.length * 48)}>
                  <BarChart data={byAgent} layout="vertical" margin={{ top: 0, right: 16, left: 8, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.15)" horizontal={false} />
                    <XAxis type="number" tick={{ fontSize: 11, fill: tickColor }} tickFormatter={formatNum} />
                    <YAxis type="category" dataKey="agent_name" tick={{ fontSize: 11, fill: tickColor }} width={90} />
                    <Tooltip
                      contentStyle={{ background: 'rgba(255,255,255,0.85)', borderRadius: 12, border: 'none', fontSize: 12 }}
                      formatter={(value: unknown) => [formatNum(Number(value ?? 0)), '']}
                    />
                    <Legend wrapperStyle={{ fontSize: 12, color: tickColor }} />
                    <Bar dataKey="output_tokens" name="输出 Tokens" fill="#06b6d4" radius={[0, 4, 4, 0]} />
                    <Bar dataKey="input_tokens" name="输入 Tokens" fill="#6366f1" radius={[0, 4, 4, 0]} />
                  </BarChart>
                </ResponsiveContainer>
                ...
                <div className="mt-4 overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {byAgent.map((a) => (
                        <tr key={a.agent_id} className="border-b border-gray-100 dark:border-zinc-800/50 hover:bg-gray-50 dark:bg-zinc-900 transition-colors">
                          <td className="py-2 text-gray-800">{a.agent_name}</td>
                          <td className="py-2 text-right text-gray-700">{formatNum(a.input_tokens)}</td>
                          <td className="py-2 text-right text-gray-700">{formatNum(a.output_tokens)}</td>
