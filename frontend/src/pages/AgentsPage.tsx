...
const AgentsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    ImageToolsService.listImageToolsApiV1ImageToolsGet(0, 100)
      .then(setAvailableImageTools)
      .catch(console.error)
    BuiltinToolsService.listAvailableBuiltinToolsApiV1BuiltinToolsGet()
      .then(setAvailableBuiltinTools)
      .catch(console.error)
  ...
  const openAgentModal = async (agent: AgentOut | null = null) => {
    if (agent) {
      try {
        ...
        const [fullAgent, boundApiToolIds, boundImageToolIds, boundBuiltinToolTypes] =
          await Promise.all([
  ...
  const createSession = (agentId: number) => {
  ...
  const saveAgent = async () => {
    try {
      ...
      await Promise.all([
        AgentApiToolsService.syncAgentToolsApiV1AgentsAgentIdApiToolsPost(agentId, {
          tool_ids: api_tool_ids,
        }),
        AgentImageToolsService.syncAgentImageToolsApiV1AgentsAgentIdImageToolsPost(agentId, {
          tool_ids: image_tool_ids,
        }),
        AgentBuiltinToolsService.syncAgentBuiltinToolsApiV1AgentsAgentIdBuiltinToolsPost(agentId, {
          ...
          {agents.map((agent) => (
            ...
            >
              ...
              <div className="flex items-center justify-between text-xs text-gray-600 mb-4">
                <span className="px-3 py-1.5 bg-gray-100 dark:bg-zinc-800 rounded-full">
                  上下文 Token 上限: {agent.max_token_size ?? 0}
                </span>
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    onClick={(e) => {
                      e.stopPropagation()
                      void openAgentModal(agent)
                    }}
