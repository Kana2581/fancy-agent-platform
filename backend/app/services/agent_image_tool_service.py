...
from app.mappers.agent_image_tool_mapper import AgentImageToolMapper


class AgentImageToolService:

    def __init__(self, db: AsyncSession):
        self.db = db
        self.mapper = AgentImageToolMapper(db)

    async def list_tools_for_agent(self, agent_id: int) -> List[int]:
        rows = await self.mapper.list_by_agent_id(agent_id)
        return [r.image_tool_id for r in rows]
